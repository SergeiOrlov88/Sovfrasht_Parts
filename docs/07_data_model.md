...
**Notification** *(добавлено на шаге 7)* — уведомление внутри приложения (FR-NOT-01): получатель, тип события (`expert_resolved` и т.п.), заголовок, текст, `payload` со ссылками на скан/деталь и отметка о прочтении. Канал MVP — только in-app; точка расширения под email оставлена в сервисе (`send_email_stub`).

У **ModerationTask** на шаге 7 добавлено `claimed_at` — момент взятия задачи в работу. Вместе с `created_at` и `resolved_at` даёт SLA (FR-HITL-04): сколько задача ждала очереди и сколько заняло само решение. Метрика считается на чтении, отдельным полем не дублируется.
