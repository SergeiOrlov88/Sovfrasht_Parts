...
**Цена предложения — строка.** В `SupplierOffer.price` и в ответе `GET /parts/{id}/offers` цена хранится и отдаётся строкой (`"$1 190"`, `"€820"`). Для MVP это осознанное упрощение: предложения курируемые и демонстрационные, точные сравнения по ним не делаются, а формат записи у разных источников разный.

**Ограничение:** сортировка по цене работает эвристикой (из строки извлекаются цифры), сравнение между валютами невозможно, арифметика (например, «ремонт = 50% от замены») даёт приблизительный результат.

**Когда исправляем:** при подключении реального поставщика по API (FR-PRO-05). Тогда `price` разделяется на числовое `price_amount` (decimal) и `price_currency` (ISO 4217), а строковое представление остаётся отдельным полем для отображения «как у поставщика». Миграция затронет `supplier_offers` и схему `OfferRead`.
